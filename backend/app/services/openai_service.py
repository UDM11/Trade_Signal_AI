...
def generate_explanation(prediction: str, confidence: float, indicators: dict, news_text: str = "") -> dict:
    client = _get_client()
    ...
    if client is None:
        return _build_result(
            explanation     = "OpenAI API not configured. Using model-calculated price levels based on technical indicators.",
...
{chr(10) + "News Context: " + news_text if news_text else ""}
...
6. estimated_days: integer between 2 and 90 — derive from ATR Rs. {atr_approx:.2f}/day and ADX {adx:.1f}:
   ...
   • Never default to 10–15 days without reasoning from these inputs
...
{{
  "explanation": "<4 sentences: (1) current market structure and trend context including ADX/EMA signals, (2) key technical reason for {prediction} signal, (3) exact entry Rs. price, T1 and T2 targets with % upside/downside, stop loss level, (4) timeline reasoning based on ATR and ADX, plus most important risk>",
